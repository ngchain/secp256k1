...
func (curve *secp256k1.KoblitzCurve) getDoublingPoints() [][3]secp256k1.fieldVal {
	doublingPoints := make([][3]secp256k1.fieldVal, curve.BitSize)

	// initialize px, py, pz to the Jacobian coordinates for the base point
	px, py := curve.bigAffineToField(curve.Gx, curve.Gy)
	pz := new(secp256k1.fieldVal).SetInt(1)
	for i := 0; i < curve.BitSize; i++ {
		doublingPoints[i] = [3]secp256k1.fieldVal{*px, *py, *pz}
		// P = 2*P
...
// secp256k1.go.
func (curve *secp256k1.KoblitzCurve) SerializedBytePoints() []byte {
	...
	for byteNum := 0; byteNum < curve.byteSize; byteNum++ {
		...
		for i := 0; i < 256; i++ {
			px, py, pz := new(secp256k1.fieldVal), new(secp256k1.fieldVal), new(secp256k1.fieldVal)
			for j := 0; j < 8; j++ {
				if i>>uint(j)&1 == 1 {
...
func (curve *secp256k1.KoblitzCurve) EndomorphismVectors() (a1, b1, a2, b2 *big.Int) {
